...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Starting Cryptocurrency news fetching...');
    
    const allArticles = [];

    // Fetch news for Cryptocurrencies (2 API calls to get 40 articles)
    for (let apiCall = 0; apiCall < 2; apiCall++) {
      console.log(`Making Cryptocurrency API call ${apiCall + 1}/2...`);
      
      const newsResponse = await fetch(
        `https://api.marketaux.com/v1/news/all?symbols=${MAJOR_CRYPTOCURRENCIES.join(',')}&filter_entities=true&language=en&limit=20&page=${apiCall}&api_token=${marketauxApiKey}`
      );

      if (!newsResponse.ok) {
        console.error(`MarketAux API error for Cryptocurrencies: ${newsResponse.status}`);
        continue;
      }
      ...
      console.log(`Fetched ${articles.length} Cryptocurrency articles from API call ${apiCall + 1}`);
    ...
    for (const article of allArticles) {
      ...
      if (!symbol || !MAJOR_CRYPTOCURRENCIES.includes(symbol)) continue;
    ...
    console.log(`Unique Cryptocurrency articles: ${uniqueArticles.length}`);
    ...
    // Process main analysis articles (one per crypto)
...
You are a professional cryptocurrency analyst. Analyze this news article about ${symbol} and provide a JSON response:
...
`;
          ...
          const chatResponse = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
              ...
              messages: [
                {
                  role: 'system',
                  content: 'You are a professional cryptocurrency analyst. Provide clear, actionable market analysis in valid JSON format only.'
                },
                {
                  role: 'user',
          ...
          if (chatResponse.ok) {
            ...
            const processedArticle = {
              ...
              category: analysis.category || 'Cryptocurrency',
              url: article.url,
    ...
    // Create historical analysis for cryptos without current news
    ...
    for (const article of uniqueArticles) {
      ...
      if (symbol && MAJOR_CRYPTOCURRENCIES.includes(symbol)) {
        ...
        try {
          const headlinePrompt = `Analyze this cryptocurrency article and provide JSON response:
{
...
Symbol: ${symbol}`;
          ...
          const headlineResponse = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
              model: 'gpt-4o-mini',
              messages: [
                { role: 'system', content: 'You are a cryptocurrency analyst providing quick market sentiment analysis.' },
    ...
    console.log('Storing Cryptocurrency articles in database...');
    
    // Clear existing Cryptocurrency articles
    for (const symbol of MAJOR_CRYPTOCURRENCIES) {
    ...
    const summary = {
      ...
      message: 'Cryptocurrencies processed successfully'
    };

    console.log(`🎉 Successfully processed ${summary.mainAnalyses} main analyses and ${summary.additionalHeadlines} additional headlines for Cryptocurrencies`);
