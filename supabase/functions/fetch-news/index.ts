...
serve(async (req) => {
  ...
  try {
    ...
    console.log('ENV keys:', {
      hasMarketaux: !!marketauxKey,
      hasOpenAI: !!openAIApiKey
    });

    if (!marketauxKey || !openAIApiKey) {
    ...
    // Build Marketaux URL
    const symbols = ['AAPL','TSLA','NVDA','MSFT','GOOGL'];  // adjust or derive dynamically
    const newsUrl = `https://api.marketaux.com/v1/news/all?symbols=${symbols.join(",")}&api_token=${marketauxKey}&language=en&filter_entities=true&limit=15`;

    console.log('Fetching news from Marketaux:', newsUrl);
    const newsResponse = await fetch(newsUrl);
    if (!newsResponse.ok) {
      const errorText = await newsResponse.text();
      console.error('Marketaux API error:', newsResponse.status, errorText);
      throw new Error(`Marketaux API error: ${newsResponse.status} - ${errorText}`);
    }
    ...
    for (const article of newsData.data.slice(0, 10)) {
      try {
...
`;
  ...
  try {
    ...
    return {
      ...
      sentiment: "Neutral",
      priority: "MEDIUM",
      reasoning: "Parsing error: default fallback"
    };
  }
}
