...
const MoverCard = ({ stock, isGainer, rank }: { stock: any; isGainer: boolean; rank: number; }) => {
  ...
  return (
    <Card
      className={`bg-slate-800/50 backdrop-blur hover:border-emerald-500/30 transition-all ${getRankColor(rank)}`}>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              {getRankIcon(rank)}
              <Badge className={`${isGainer ? 'bg-emerald-500' : 'bg-red-500'} text-white`}>{stock.symbol}</Badge>
            </div>
            <div>
      ...
      <CardContent className="pt-0">
        ...
        {showHeadlines && stock.headlines?.length > 0 && (
          <div className="space-y-2 mt-3 pt-3 border-t border-slate-700">
            ...
            {stock.headlines.map((headline: any, index: number) => (
              <a
                ...
                <p className="text-slate-400 text-xs mb-2">Impact: {headline.summary}</p>
                <div className="flex items-center justify-between">
        ...
        <div className="max-w-7xl mx-auto px-6 py-12">
          ...
          <div className="mb-8">
            ...
            <div className="flex items-center gap-2 mt-3">
              <Badge variant="secondary" className="bg-emerald-500/20 text-emerald-400">LIVE DATA</Badge>
              ...
              {moversData.gainers.length > 0 && (
                <section className="mb-8">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <Badge className="bg-emerald-500 text-white">Ranked by % gain</Badge>
                  </div>
                  <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                    {moversData.gainers.map((stock, index) => (
                      <MoverCard key={stock.symbol} stock={stock} isGainer rank={index + 1} />
                    ))}
                  </div>
                </section>
              )}
              ...
              {moversData.losers.length > 0 && (
                <section>
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <Badge className="bg-red-500 text-white">Ranked by % loss</Badge>
                  </div>
                  <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
