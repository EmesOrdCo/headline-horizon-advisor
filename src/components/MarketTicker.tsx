...
const MarketTicker = () => {
  ...
  if (isLoading) {
    return (
      <div className="fixed top-[58px] left-0 right-0 z-40 bg-theme-ticker border-none">
        <div className="w-[95%] mx-auto py-2">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 bg-theme-accent rounded-full"></div>
              <span className="text-theme-accent text-sm font-medium">LIVE MARKETS</span>
            </div>
            <div className="text-theme-secondary text-sm font-medium">Loading live market data...</div>
          </div>
        </div>
      </div>
    );
      ...
      <div className="fixed top-[58px] left-0 right-0 z-40 bg-theme-ticker border-none">
        <div className="w-[95%] mx-auto py-2">
          <div className="flex items-center gap-4">
            ...
            <div className="text-theme-secondary text-sm font-medium">Unable to fetch live market data</div>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed top-[58px] left-0 right-0 z-40 bg-theme-ticker border-none overflow-hidden">
      <div className="w-[95%] mx-auto py-2">
        <div className="flex items-center gap-4">
            ...
            <div className="animate-scroll flex gap-8">
              {duplicatedData.map((item, index) => (
                <div key={`${item.symbol}-${index}`} className="flex items-center gap-4 whitespace-nowrap">
                  <span className="text-theme-primary font-semibold text-sm">{item.symbol}</span>
                  <span className="text-theme-secondary text-sm">${item.price.toFixed(2)}</span>
                  <span className={`text-sm flex items-center gap-1 ${
                    item.change >= 0 ? 'text-theme-up' : 'text-theme-down'
                  }`}>
                    <Triangle 
                      className={`w-3 h-3 ${
                        item.change >= 0 
                          ? 'text-theme-up fill-current' 
                          : 'text-theme-down fill-current rotate-180'
                      }`} 
                    />
